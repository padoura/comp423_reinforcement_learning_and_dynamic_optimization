...
Finally, as an extra sanity check, I pitted the Random and Threshold agents against each other in the same experimental setup (5000000 games). Even though the Threshold agent is a more rational player and still beats the Random agent, the results (mean payoffs: 0.3093, std: 1.643) clearly shows that it is far from being the optimal policy. As we will see in the following sections (and Table \ref{table:random-results}), Policy Iteration indeed yielded the best policy out of all policies shown in this project.
