...
class RandomAgent:
    ...
    def calculate_state_space(self, win_probabilities, loss_probabilities, flop_probabilities, range_probabilities):
        ...
        # | 'opponent_range' |   <22  |Possible range of opponent's hand, meaningful for ThresholdAgent, else 'AK' |
        ...
        if game_round == 1: # end of round 1
            ...
            if new_other_chips != 0:
                ...
                for new_other_chips in new_other_chips_round2_list:
                    for public_cards in flop_probabilities[hand]['AK']:
                        self._add_or_update_key(state_space, full_key, flop_probabilities[hand]['AK'][public_cards]*action_prob, my_action, position, new_my_chips, new_other_chips, is_terminal, reward, hand, public_cards)
        else: # end of round 2
            for public_cards in flop_probabilities[hand]['AK']:
                full_key = key + public_cards + '_AK'
                if new_other_chips == 0:
                    # game finished, result based on players' hands 
                    is_terminal = True
                    win_prob = win_probabilities[hand][public_cards]['AK']
                    loss_prob = loss_probabilities[hand][public_cards]['AK']
    ...
    def _add_or_update_key(self, state_space, key, prob, my_action, position, new_my_chips, new_other_chips, is_terminal, reward, hand, public_cards):
        if prob > 0:
            ...
            new_key = position + '_' + str(new_my_chips) + '_' + str(new_other_chips) + '_' + hand + '_' + public_cards + '_AK'
